...
static int	check_overflow(int ac, char **av, t_arguments *arguments)
{
	int	i;
...
static t_arguments	fill_arguments(int ac, char **av)
{
...
t_arguments	parser(int ac, char **av)
{
	...
	arguments.valid = 0;
	if (int_parser(ac, av) || str_parser(av[8]))
		return (arguments);
	return (fill_arguments(ac, av));
}
